...
namespace Embodiment.Native
{
    ...
    internal unsafe struct BufferHandler<T> : IDisposable where T : unmanaged
    {
        ...
        {
            if (!Allocated) throw new ObjectDisposedException("Cannot copy, the buffer has been disposed.");
        ...
        {
            if (!Allocated) throw new ObjectDisposedException("Cannot copy, the source buffer has been disposed.");
            if (!buffer.Allocated) throw new ObjectDisposedException("Cannot copy, the destination buffer has been disposed.");
