...
public static class SpeechManager
{
   ...
   // public static void SetChatGPTPrompt(string prompt) { _prompt = prompt; }
    ...
    public static async Task ReadMessageAsync()
    {
        ...
        if (_messageToRead.Length != 0)    // only if the text contains something
        {
            OnNewSpokenText?.Invoke(_messageToRead);   // Fires off an event with the converted ChatGPT message
            using (speechSynthesizer)
            {
                ...
                using (var result = await speechSynthesizer.SpeakSsmlAsync(GetStyledVoiceString(_messageToRead)))
                {
                    var audioDataStream = AudioDataStream.FromResult(result);
                    var isFirstAudioChunk = true;
                    var audioClip = AudioClip.Create(
                        "Speech",
                        24000 * 600, // Can speak 10mins audio as maximum
                        1,
                        24000,
                        true,
                        (float[] audioChunk) =>
        ...
        //    {
        //        // Reads out the text we got from ChatGPT
        //        var speechSynthesisResult = await speechSynthesizer.SpeakSsmlAsync(GetStyledVoiceString(_messageToRead));
        //        OutputSpeechSynthesisResult(speechSynthesisResult, _messageToRead); // literally just Debug.Logs :D
        //    }
        //}
    }
